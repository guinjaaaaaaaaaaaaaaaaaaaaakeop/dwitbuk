...
| (as chongdae's `reviewer`) | The review placed by the runner: a review that waits to be asked for is never asked for (a site went 22 runs without one). Declare `"reviewer": ["python3", "{plugin:dwitbuk}/dwitbuk.py", "review", "--target", "."]` in hunsu.json `roles` (an argv, like `coherence` — not `dwitbuk:review`, which would be judged as a worker's prompt) and lock; chongdae runs it when a run ends (`close`, `complete`), and `reviews/<id>.json` goes into that run's close commit. One review per run, since the previous one's head |
| (SessionStart hook) | One fact when there is one: `dwitbuk: N undisposed objection(s) in <review> (kinds)` and the `follow`/`dispose` commands — what the reviews the runner placed still need from a person. Nothing open, no reviews, or a worker session: silent. Never blocks, never writes |
| (Stop hook) | The eyes placed by the environment: when the project sets `settings.dwitbuk.stop-eyes: true` in hunsu.json (read there, with this machine's overlay from hunsu.local.json on top; a lock that carries it still counts), a session that tries to end with a dirty tree gets its diff read (lens `fresh`, the plan as contract if there is one); findings refuse the stop once and reach the session, and land in the latest review. One model call per stop with changes — the team's switch, not the plugin's default. Inside a worker session (`AGENT_WORKER=1`: the eyes, a judge, a builder) the hook does nothing, since the host runs the project's hooks in those sessions too |
...
The latest review is the open set. A finding seen again — the same kind, `where` and text, the changing counts struck — inherits its disposition; an objection that stops recurring without one is carried as `undisposed`; an observation that stops recurring simply stops. An observation is written in full once: the reviews after it count it in one `unchanged` line naming the review that holds it (`seen-in`), and read that review to recognize it again — so a project's hundred delegated relations are on record once, not in every review (they were 58 KB of every 96 KB review on a site). A fact whose text changed is a new fact, written in full. `eyes request` puts its packet and the worker's transcript under `--out` (default `dwitbuk-eyes/`; a reviewer keeps them under `reviews/`).
